(ns elecequipmfg.operation-test
  "Smoke tests for the compiled ElecEquipOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `elecequipmfg.governor-contract-test`; the Store contract in
  `elecequipmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "ElecEquipOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "winder-001" :severity :moderate :description "high-voltage test anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
