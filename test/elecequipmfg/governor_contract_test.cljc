(ns elecequipmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control winding/assembly/test-bench
  equipment directly... does NOT authorize or execute maintenance/
  equipment operations... does NOT self-issue an electrical-safety
  certification mark') implemented faithfully. The single invariant
  under test:

    ElecEquipAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Electrical Equipment Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :transformer}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :transformer (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-winding-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "winder-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
