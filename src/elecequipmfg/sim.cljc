(ns elecequipmfg.sim
  ...
  production quantity, a proposal that tries to ACTUATE winding/
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on winder-001 (verified, registered, winding machine -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on winder-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-3 on winder-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
