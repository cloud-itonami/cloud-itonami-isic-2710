(ns elecequipmfg.store
  "SSoT for the electric motor/generator/transformer/distribution-and-
  control-apparatus plant-operations coordination actor, behind a
  ...
  `resinmfg.store`, `cloud-itonami-isic-2211`'s own `tyremfg.store`),
  ...
  self-contained (no external electrical-equipment-manufacturing
  ...
  Four kinds of entity live here:
    ...
    - `equipment`         -- a winding/assembly/test-bench unit's own
    ...
    - `maintenance`       -- a scheduled winding/assembly/test-bench
                             maintenance-window DRAFT against a piece
                             of equipment (`elecequipmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound electrical-equipment
                             shipment DRAFT (`elecequipmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (elecequipmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (elecequipmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :transformer
                :model "TX-500KVA" :dielectric-test-kv 250.0
                :quantity-units 500.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :generator
                :model "GEN-200KW" :dielectric-test-kv 40.0
                :quantity-units 200.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 180.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :electric-motor
                :model "MTR-15KW" :dielectric-test-kv 5.0
...
(defn- sample-equipment []
  {"winder-001" {:id "winder-001" :kind :winding-machine
                 :verified? true :registered? true
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `elecequipmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `elecequipmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  winding-machine unit (schedulable for maintenance), one UNVERIFIED/
  unregistered dielectric-test-bench unit (blocks any maintenance
