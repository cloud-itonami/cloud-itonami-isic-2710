(ns elecequipmfg.registry
  "Pure-function domain logic for the electric motor/generator/
  transformer/distribution-and-control-apparatus plant-operations
  ...
  coordinated shipment), not the act of actuating winding/assembly/
  test-bench equipment or dispatching a real freight carrier, and
  never the act of issuing an electrical-safety certification mark
  (this actor NEVER does any of those -- see README `What this actor
  does NOT do`).

  SCOPE: ISIC 2710 covers manufacture of electric motors, generators,
  transformers and electricity distribution and control apparatus --
  coil winding, insulation, assembly, and testing (including high-
  voltage dielectric/hipot withstand testing) lines producing finished
  electric motors, generators, transformers, and electricity
  distribution/control apparatus. This actor coordinates the
  ...
  coordination) -- it never touches the winding/assembly/test-bench
  equipment directly, and it never stands in for the certification
  body that issues electrical-safety compliance marks (e.g. UL/CE/
  IEC).")
...
(def valid-product-types
  ...
  #{:electric-motor :generator :transformer
    :distribution-apparatus :control-apparatus})

(def dielectric-test-kv-min
  "Physical floor for a batch's own dielectric (hipot/withstand) test
  voltage reading, in kV. A test voltage is never negative."
  0.0)

(def dielectric-test-kv-max
  "Physical ceiling for a batch's own dielectric (hipot/withstand) test
  voltage reading, in kV. Grounded in IEC 60076-3's lightning-impulse
  withstand table, which tops out around 2400 kV peak for the highest
  standard power-transformer voltage class (Um = 1200 kV) -- a reading
  above this is implausible sensor/QC data, not a real routine
  dielectric test on any standardized class of electrical equipment."
  2500.0)

(def defect-rate-min-percent
  "Physical floor for a batch's own winding/assembly/test defect-rate
  reading (zero defects is the best possible outcome, never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own winding/assembly/test defect-rate
...
(defn dielectric-test-kv-valid?
  "Is `kv` a physically plausible dielectric (hipot/withstand) test
  voltage reading, in kV? Rejects nil, non-numbers, negative values,
  and values beyond `dielectric-test-kv-max` -- a fabricated or
...
(defn defect-rate-valid?
  "Is `percent` a physically plausible batch winding/assembly/test
...
(defn register-maintenance
  ...
  winding/assembly/test-bench-equipment maintenance window against a
  verified, registered piece of equipment. Pure function -- does not
  actuate the winding/assembly/test-bench equipment or execute any
  maintenance; it builds the RECORD a plant coordinator would keep.
  `elecequipmfg.governor` independently re-verifies the equipment's own
...
(defn register-shipment
  ...
  `elecequipmfg.governor` independently re-verifies the shipment's own
