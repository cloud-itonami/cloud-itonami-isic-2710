(ns elecequipmfg.advisor
  "ElecEquipAdvisor -- the *contained intelligence node* for the
  electric motor/generator/transformer/distribution-and-control-
  apparatus plant-operations coordination actor.

  It normalizes production-batch patches (product-type/dielectric-
  test-kv/quantity/defect-rate), drafts a winding/assembly/test-bench-
  ...
  a committed record and NEVER a real winding/assembly/test-bench-
  equipment actuation, freight dispatch, or electrical-safety
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `elecequipmfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `elecequipmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct winding/
  ...
  `elecequipmfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [elecequipmfg.registry :as registry]
...
(defn- schedule-maintenance
  "Draft a winding/assembly/test-bench-equipment maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `elecequipmfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft an insulation-failure/electrical-safety/high-voltage-test-
  ...
  technicality). See `elecequipmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `elecequipmfg.governor` also always
...
(defn- coordinate-shipment
  ...
  `elecequipmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは電気機器(モータ・発電機・変圧器・配電制御装置)製造工場"
       ...
       "巻線・組立・試験等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `elecequipmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :elecequipmfg-advisor-proposal
   :op         (:op request)
