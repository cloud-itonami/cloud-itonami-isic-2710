(ns elecequipmfg.operation
  "ElecEquipOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (ElecEquipAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Electrical
  ...
  (:require [langgraph.graph :as g]
            ...
            [elecequipmfg.advisor :as advisor]
...
(defn build
  "Compiles an ElecEquipOperationActor graph bound to `store`.
  opts:
    :advisor      -- an `elecequipmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ElecEquipAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Electrical Equipment Plant Operations Governor -- independent censor.
      (g/add-node :govern
