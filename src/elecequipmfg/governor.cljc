(ns elecequipmfg.governor
  "Electrical Equipment Plant Operations Governor -- the independent
  compliance layer that earns the ElecEquipAdvisor the right to
  ...
  draft-schedule) winding/assembly/test-bench equipment, whether a
  ...
  a coordination proposal and becomes direct winding/assembly/test-
  ...
  *reject* a proposal and fall back to HOLD.
  ...
  `:electrical-equipment-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:winder/actuate` or
                                       `:test-bench/run`) is the 'direct
                                       winding/assembly/test-bench-
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct winding/
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct winding/assembly/test-bench-equipment control,
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は巻線・組立・試験設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate winding/assembly/test-bench equipment -- this
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "巻線・組立・試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
